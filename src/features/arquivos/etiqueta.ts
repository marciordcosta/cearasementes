 ...
 * Espécie + Cultivar, ver interpretarConteudoLaudo.ts), sem precisar de
 * cadastro à parte: descarta as palavras da Espécie do início (ex.:
 * "Andropogon Gayanus" = 2 palavras), sobrando só o Cultivar. Sem Espécie
 * conhecida (laudo antigo, ou modelo de documento que não destaca Espécie à
 * parte), assume 1 palavra de gênero só (comportamento antigo, ex.:
 * "Andropogon Planaltina" -> descarta só "Andropogon"). Se alguma palavra do
 * que sobrar contiver "incrustad" (o Processo já vem à parte de laudo.processo,
 * isso aqui só limpa o Cultivar), essa palavra sai do Cultivar.
 */
export function derivarCultivar(nomeProduto: string, especie?: string | null): string {
  ...
  return semGenero.filter((p) => !/incrustad/i.test(p)).join(' ');
}
...
export function montarDadosEtiqueta(laudo: ArquivoLaudo, produtos: ProdutoParametrizacao[], produtosPreco: Produto[]): DadosEtiqueta {
  const cultivar = derivarCultivar(laudo.nomeProduto, laudo.especie);
